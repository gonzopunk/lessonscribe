...
3. **`onDragOver` handles both kinds** — proceed for `"template"` and `"instance"`, return early otherwise. Existing hovered-instance resolution and before/after side computation (active rect center vs. over rect center) is unchanged. Extra guard for instance drags: if the hovered instance's `dayKey` equals the source day read directly from `e.active.data.current.dayKey` (not the `dragSourceDayKey` state, which may not have flushed before the first `onDragOver`), clear `dragOverPosRef.current` / `dragOverPos` and return — same-day reordering already previews via the sortable strategy. The `dragSourceDayKey` state is still used for the DayCell prop.
