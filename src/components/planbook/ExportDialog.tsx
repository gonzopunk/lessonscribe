...
export function ExportDialog({ open, onOpenChange }: Props) {
  ...
  const exportNow = () => {
    ...
    openPrintWindow({
      title,
      courseColorHex: primaryHex,
      bodyHTML: sectionsHtml.join("") || "<p>No plans in range.</p>",
      fontFamily: profile.fontFamily,
      orientation: profile.orientation,
      pageNumbers: profile.sections.pageNumbers,
      runningHeader: profile.sections.runningHeader,
      runningHeaderText: profile.header.teacherName
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Pick a preset or customize every section, then print or save as PDF.
          </DialogDescription>
        </DialogHeader>
